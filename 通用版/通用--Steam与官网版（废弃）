...
;===TFM游戏活动提醒脚本(通用版）===
;===识别到左上角活动地图代号(固定出现的字符),执行末尾的操作===
;===绑定Tfm游戏窗口(使用版本为Steam或官网版本)===
;===禁止多开Steam或官网版本(包括同时使用2版本），否则无法识别===
;===如需多账号使用（不使用本脚本），可以用支持flash的浏览器（CEF浏览器、360极速浏览器）、flash大厅（最好不用）等
;===已知bug：===
;===1.房间列表-创建新房间-游戏模式选项Transformice会被错误识别[修改方法:识别内容加长到@(修复后把3718行时间改长)]===

SetTitleMatchMode 2
id:=WinExist("Transformice ahk_class ApolloRuntimeContentWindow")
FindText().BindWindow(id,4)  ; 解绑窗口使用 FindText().BindWindow(0)

; #Include <FindText>

t1:=A_TickCount, Text:=X:=Y:=""

Text:="|<>**50$81.000000S0003U07z000060000I00U80000a0003U07jzyzbxzrzzxyy5+ER7V33d16sQMdDtbPhnAtaqPR5/DhfTexhRqrrcdH1dQBIh+eqW15+Lh/xexdJKprsdGxdPhHB+equT5+MB/1f3dJKsM8tlztzzDrCvxzzU"

if (ok:=FindText(&X, &Y, 989-150000, 243-150000, 989+150000, 243+150000, 0, 0, Text))
{
...
WinActivate "Transformice" ;激活TFM窗口

Sleep 1000 ;暂停时间(毫秒)

Send "{Right}" ;按键操作一次，防止挂机死(最好用方向键'UP\Down\Left\Right'，而不是用'WASD';防止卡输入法)

SoundBeep 750, 3000 ;声音提醒；参数1数字越大，声音越尖，越小相反，限定区间(37-32767)；参数2为时间(毫秒)

;MsgBox("找到:`t" (IsObject(ok)?ok.Length:ok)
;  . "`n`n时间:`t" (A_TickCount-t1) " 毫秒"
;  . "`n`n位置:`t" X ", " Y
;  . "`n`n结果:`t<" (IsObject(ok)?ok[1].id:"") ">", "Tip", 4096) ;脚本运行时间(已注释)

;换房
;Send "{Enter}{NumpadDiv}room{Space}vanilla1{Enter}"
;Sleep 150
